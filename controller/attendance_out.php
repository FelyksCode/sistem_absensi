...
if (!$result) {
    ...
    $_SESSION['error'] = "Anda harus melakukan absen masuk terlebih dahulu.";
...
if ($result) {
    ...
    $_SESSION['error'] = "Anda sudah melakukan absen keluar hari ini.";
    header('Location: ../index.php');
    exit();
}
...
$query = "INSERT INTO attendance (user_id, longitude, latitude, type, created_at) VALUES (:user_id, :longitude, :latitude, 'out', NOW())";
$stmt = $handle->prepare($query);
$stmt->bindParam(":user_id", $user_id);
...
if ($stmt->execute()) {
    ...
    $_SESSION['success'] = "Absen keluar berhasil.";
} else {
    // Error occurred
    $_SESSION['error'] = "Gagal melakukan absen keluar. Silakan coba lagi.";
}
